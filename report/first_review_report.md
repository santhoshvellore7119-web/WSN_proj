...
- **Empirical Findings & Trade-Off Analysis:**
  ...
  - In extreme occlusion ($p_{\text{shadow}} = 1.0$), where active relays suffer heavy solar deprivation, Time-Augmented DP's lookahead routing actively protects vulnerable forwarders, extending network operational lifetime on the canonical topology by **+6 rounds** (FND 102 vs 96, +1.2% residual energy gain) and maintaining $\text{FND} = 98.5 \pm 6.5$ vs $97.9 \pm 7.4$ across $N=30$ seeds (with topology-specific gains up to $+19$ rounds).
